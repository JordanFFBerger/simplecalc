...
void main() {
  ...
  test('operations chain from left to right', () {
    enter('2'); calculator.choose('+'); enter('3');
    calculator.choose('*'); enter('4'); calculator.equals();
    expect(calculator.value, '20');
  });

  test('operator can be replaced before the next operand', () {
    enter('9'); calculator.choose('+'); calculator.choose('-');
    enter('2'); calculator.equals();
    expect(calculator.value, '7');
  });

  test('division by zero is recoverable with a new number', () {
    enter('8'); calculator.choose('/'); enter('0'); calculator.equals();
  ...
  test('editing, duplicate decimals, and clear', () {
    enter('1.2.3'); expect(calculator.value, '1.23');
    calculator.backspace(); expect(calculator.value, '1.2');
    calculator.sign(); expect(calculator.value, '-1.2');
    calculator.clear(); expect(calculator.value, '0');
    expect(calculator.operator, isNull);
  });

  test('negative operands including negative fractions', () {
    enter('2'); calculator.choose('*'); calculator.sign();
    enter('.5'); calculator.equals(); expect(calculator.value, '-1');
    calculator.clear(); calculator.sign(); enter('5');
    expect(calculator.value, '-5');
  });

  test('typing after equals starts a new calculation', () {
    enter('4'); calculator.choose('+'); enter('3'); calculator.equals();
    enter('2'); expect(calculator.value, '2');
    expect(calculator.expression, isEmpty);
  });
}
